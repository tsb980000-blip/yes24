...
# 스크립트 위치 기준으로 워크스페이스 루트의 .env 로드
...
if not api_key:
    raise ValueError(f"GEMINI_API_KEY가 설정되지 않았습니다. {dotenv_path} 파일을 확인해 주세요.")
...
# 제미나이 모델 초기화 (gemini-1.5-flash 모델 사용)
...
@app.get("/")
def read_root():
    return {"message": "kakaobot 스킬 서버가 정상 동작 중입니다."}
